...
public class LoopBoundPrecisionAdjustment implements PrecisionAdjustment {
  ...
  @Option(
    secure = true,
    description = "threshold for adjusting the threshold for unrolling loops of the program (0 is infinite).\n"
        + "only relevant in combination with a non-static maximum loop iteration adjuster.")
  private int maxLoopIterationsUpperBound = 0;

  @Option(
    secure = true,
    description = "this option controls how the maxLoopIterations condition is adjusted when a condition adjustment is invoked.")
  ...
  // FIXME: Write more precise description
  @Option(
    secure = true,
    description = "threshold for adjusting the threshold for unrolling loops of the program, until the "
        + "program is interrupted and an external invariant generation tool is called  (0 is infinite).\n"
        + "only relevant in combination with a non-static invGen maximum loop iteration adjuster.")
  private int maxLoopIterationsUntilInvGenCalled = 0;
  ...
  public Optional<PrecisionAdjustmentResult> prec(
    ...
    LoopBoundPrecision adjustedPrecision =
        precision.withMaxLoopIterations(maxLoopIterations)
    ...
    LoopBoundState adjustedState =
        state.setStop(maxLoopIterations > 0 && state.getDeepestIteration() > maxLoopIterations)
  ...
  private static interface MaxLoopIterationAdjusterFactory {
    ...
    MaxLoopIterationAdjuster
        getMaxLoopIterationAdjuster(LoopBoundPrecisionAdjustment pPrecisionAdjustment);
  ...
  private static enum MaxLoopIterationAdjusters implements MaxLoopIterationAdjusterFactory {
    ...
    STATIC {
      ...
      @Override
      public MaxLoopIterationAdjuster
          getMaxLoopIterationAdjuster(LoopBoundPrecisionAdjustment pPrecisionAdjustment) {
    ...
    EXT_INV_GEN_AND_INCREMENT {

      @Override
      public MaxLoopIterationAdjuster
          getMaxLoopIterationAdjuster(LoopBoundPrecisionAdjustment pPrecisionAdjustment) {
        return new ExtInvAndIncrementalLoopIterationAdjuster(pPrecisionAdjustment);
      }
  ...
  private static class ExtInvAndIncrementalLoopIterationAdjuster
      implements MaxLoopIterationAdjuster {

    private final LoopBoundPrecisionAdjustment precisionAdjustment;

    public ExtInvAndIncrementalLoopIterationAdjuster(
    ...
    @Override
    public boolean canAdjust(int pCurrentValue) {
      if (pCurrentValue > precisionAdjustment.maxLoopIterationsUntilInvGenCalled) {
        return false;
      }
      return precisionAdjustment.maxLoopIterationsUpperBound <= 0
          || pCurrentValue < precisionAdjustment.maxLoopIterationsUpperBound;
    ...
    public boolean invGenCallNeeded(int pCurrentValue) {
      return pCurrentValue >= precisionAdjustment.maxLoopIterationsUntilInvGenCalled;
    }
  ...
  public boolean nextState() {
    ...
    if (maxLoopIterationAdjuster.canAdjust(getMaxLoopIterations())) {
      ...
      logger.log(Level.WARNING, "Adjusting maxLoopIterations to " + adjustedMaxLoopIterations);
      setMaxLoopIterations(adjustedMaxLoopIterations);
      return true;
    } else if (maxLoopIterationAdjuster instanceof ExtInvAndIncrementalLoopIterationAdjuster) {
      if (((ExtInvAndIncrementalLoopIterationAdjuster) maxLoopIterationAdjuster)
          .invGenCallNeeded(getMaxLoopIterations())) {
        logger.log(
            Level.INFO,
            "Abort and restart, in between call an external invarinat genration tool");
        return false;
      }
    }
    return false;
