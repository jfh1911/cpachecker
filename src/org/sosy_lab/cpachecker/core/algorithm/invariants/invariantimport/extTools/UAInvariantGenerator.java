...
public class UAInvariantGenerator implements ExternalInvariantGenerator {
  ...
  private Multimap<Integer, Pair<String, String>>
    ...
    pLogger.log(
        Level.INFO,
        "Starting The invariant generation for UltimateAUtomizer with timeout",
        pTimeout);
    ...
    if (!isFinished) {
      pLogger.log(
          Level.WARNING,
          "Invariant generation for Ultimate AUtomizer timed out after",
          pTimeout,
          "seconds");
      process.destroy();
    }
  ...
  public Callable<InvGenCompRes> getCallableGeneratingInvariants(
    ...
    return () -> {
      try {
        ...
        return new InvGenCompRes(res, "UltimateAutomizer");
      } catch (CPAException e) {
